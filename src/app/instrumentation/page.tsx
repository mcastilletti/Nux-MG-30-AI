...
import { Guitar as GuitarIcon, Plus, Trash2, Pencil, Speaker, Star } from 'lucide-react';
...
export default function InstrumentationPage() {
  ...
  return <AppShell><main className="mx-auto max-w-[1200px] space-y-6 py-2">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle className="flex items-center gap-2"><GuitarIcon className="h-5 w-5 text-primary" /> Chitarre</CardTitle><Button onClick={() => openGuitar()} size="sm"><Plus className="mr-2 h-4 w-4" /> Aggiungi</Button></CardHeader><CardContent className="space-y-3">
    ...
    <Dialog open={ampDialog} onOpenChange={setAmpDialog}><DialogContent><DialogHeader><DialogTitle>{editingAmp ? 'Modifica amplificatore' : 'Nuovo amplificatore'}</DialogTitle><DialogDescription>Indica il punto della catena in cui entra il segnale.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><div className="space-y-2"><Label>Modello</Label><Input value={ampModel} onChange={e => setAmpModel(e.target.value)} placeholder="Es. Fender Deluxe Reverb" /></div><div className="space-y-2"><Label>Ingresso</Label><Select value={inputPosition} onValueChange={(value: AmpInputPosition) => setInputPosition(value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="before-preamp">Prima del preamplificatore</SelectItem><SelectItem value="after-preamp">Dopo il preamplificatore</SelectItem></SelectContent></Select></div></div><DialogFooter><Button variant="outline" onClick={() => setAmpDialog(false)}>Annulla</Button><Button onClick={saveAmp} disabled={!ampModel.trim()}>Salva amplificatore</Button></DialogFooter></DialogContent></Dialog>
  </main></AppShell>;
}
